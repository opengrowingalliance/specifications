...
5. [Data Format and Schema](#5-data-format-and-schema)
   - [5.1 Primary Data Format](#51-primary-data-format)
   - [5.2 Schema Definition](#52-schema-definition)
   - [5.3 Additional Representation Formats](#53-additional-representation-formats)
   - [5.4 Time Representation](#54-time-representation)
     - [5.4.1 Time Representation Principles](#541-time-representation-principles)
     - [5.4.2 Data Types](#542-data-types)
     - [5.4.3 Validation Rules](#543-validation-rules)
     - [5.4.4 Implementation Requirements](#544-implementation-requirements)
     - [5.4.5 Examples](#545-examples)
6. [Recipe Structure](#6-recipe-structure)
   - [6.1 Recipe Components](#61-recipe-components)
   - [6.2 Recipe Versioning](#62-recipe-versioning)
   - [6.3 Recipe Validation](#63-recipe-validation)
...
15. [Appendices](#15-appendices)
   - [Appendix A: Example Minimal Recipe](#appendix-a-example-minimal-recipe)
...
The OGSP standardizes time-related data using ISO 8601 formats with extensions for scheduling. All time-related data MUST be timezone-aware and support precision control.

#### 5.4.1 Core Principles

1. **ISO 8601 Compliance**
   - All time-related data MUST use ISO 8601 formats
   - Extensions MUST maintain ISO 8601 compatibility

2. **Type Safety**
   - Strict separation of duration, schedule, and timestamp
   - No implicit conversions between types

3. **Timezone Awareness**
   - All timestamps MUST include timezone
   - UTC MUST be supported
   - Local timezone MUST be configurable

4. **Precision Control**
   - Default precision: seconds
...
1. **Duration**
   - Format: ISO 8601 duration (e.g., "PT2H30M")
   - Minimum granularity: milliseconds
   - MUST be positive
   - Example: "PT12H30M15S" (12 hours, 30 minutes, 15 seconds)

2. **Schedule**
   - Format: Simplified cron or RRULE
   - MUST include timezone
   - MUST specify start/end times
   - Example: "0 0 * * * America/Sao_Paulo"

3. **Timestamp**
   - Format: ISO 8601 datetime with timezone
   - MUST include timezone
   - Example: "2025-03-20T08:00:00-03:00"
...
2. **Schedule Validation**
   - MUST be valid cron or RRULE
   - MUST include timezone
   - MUST validate against system capabilities

3. **Timestamp Validation**
   - MUST be valid ISO 8601 datetime
   - MUST include timezone
   - MUST be in the future for scheduling
...
2. **Precision Control**
   - Default to seconds precision
   - Allow millisecond precision for critical operations
   - Truncate or round based on configuration

3. **Error Handling**
   - Clear error messages for invalid formats
   - Graceful degradation for unsupported features
   - Validation warnings for ambiguous times

#### 5.4.5 Examples

```yaml
# Duration examples
duration1: "PT12H30M" # 12 hours 30 minutes
duration2: "P1DT2H" # 1 day 2 hours

# Schedule examples
schedule1: "0 0 * * * America/Sao_Paulo" # Daily at midnight
schedule2: "RRULE:FREQ=HOURLY;INTERVAL=2;TZID=America/Sao_Paulo" # Every 2 hours

# Timestamp example
start_time: "2025-03-20T08:00:00-03:00"
```
